...
**Versie:** 0.8 — F7a filterpaneel-dropdown ontwerp toegevoegd
...
| Requirement | Status | Opmerking |
|-------------|--------|----------|
| F1 — Initiatief aanmaken | ✅ Gereed | API + template, 5 tests |
| F2 — Initiatief bewerken | ✅ Gereed | API + edit modal + wijzigingen-logboek, 22 tests |
| F3 — Hypothesen & sub-hypothesen | ✅ Gereed | Tree-structuur, leeruitkomst validatie (23 tests) |
| F4 — Dossier (notities + bestanden) | ✅ Gereed | Notities CRUD, upload/download (15 tests) |
| F5 — Stoppen met leeruitkomst | ✅ Gereed | Validatie: stop_reason verplicht (28 tests) |
| F6 — Curaties | ✅ Gereed | CRUD + items management, meerdere curaties per initiatief (18 tests) |
| F7 — Zoeken & filteren | ✅ Gereed | FTS5 + client-side filters op fase/status/horizon/MDS (14 tests) |
| F8 — Dashboard | ✅ Gereed | Stats, filters, lege staat getest (16 tests) |
| F9 — Centrale vragen | ✅ Gereed | Aparte entiteit, many-to-many met initiatieven (19 tests) |
| H2-1 — Tags | ✅ Gereed | CRUD API, op initiatieven én centrale vragen, overzichtspagina, filtering, 17 tests |
| H2-2 — MDS als entiteit | ✅ Gereed | CRUD + koppeling initiatieven, 7 teams gemigreerd |
| F10 — AI-curatie-assistent | ✅ Gereed | Model-agnostisch, narratief/suggesties/one-pager, 18 tests |

**Tech stack:** FastAPI + SQLite (FTS5) + Jinja2/HTMX + Tailwind CDN
**AI:** Model-agnostische client (OpenAI-compatible API) — configureerbaar via MODEL_URL/MODEL_NAME  
**Zie ook:** [PLAN.md](./PLAN.md) voor gedetailleerd implementatieplan.
...
- **Centrale vraag** — de "raketpunt" waaraan initiatieven hangen. Aparte entiteit met many-to-many relatie naar initiatieven: één initiatief kan meerdere centrale vragen hebben, en één vraag kan bij meerdere initiatieven horen. Initiatief zonder centrale vraag is toegestaan; het overzicht toont prominent welke initiatieven nog geen vraag hebben.
...
### In scope (MVP)
1. **Initiatieven** — CRUD met titel, omschrijving, fase, status, horizon, MDS (vrij tekstveld), eigenaar (vrij tekstveld), aanmaakdatum, laatste wijziging. Centrale vragen via many-to-many relatie (zie F9).
...
9. **Centrale vragen** — aparte entiteit met CRUD, many-to-many koppeling naar initiatieven, overzichtspagina in navigatie.
10. **AI-curatie-assistent** — model-agnostisch (OpenAI-compatible API), configureerbaar via environment variabelen, hypothese-suggesties, narratief-generatie, one-pager.

### Buiten scope (MVP, expliciet voor agent)
- Authenticatie / multi-user / rollen
- Semantische zoek (vector embeddings)
...
- **Filterpaneel als dropdown (geen filterchips):** zie F7a voor gedetailleerd ontwerpvoorstel.
- Implementatie: SQLite FTS5 of equivalent — eenvoudige full-text zoek, geen geavanceerde semantische zoek voor MVP.

### F7a — Filterpaneel-ontwerp (dropdown)

**Principe:** Filters zijn niet standaard zichtbaar als rijen chips die verticale ruimte innemen. In plaats daarvan is er één knop "Filter resultaten" die een opklapbaar paneel opent. Dit houdt het scherm strak en geeft de filters de aandacht die ze verdienen zonder constant in beeld te staan.

**Uiterlijk en gedrag:**

```
┌─────────────────────────────────────────────┐
│  Zoeken...                    [⌘K]          │  ← altijd zichtbaar
│                                             │
│  ▼ Filter resultaten (3)   [Alles wissen]  │  ← knop met badge
│  ┌─────────────────────────────────────────┐│
│  │ Fase               Status              ││
│  │ ○ Verkenning       ○ Actief            ││
│  │ ● Experiment       ○ Gestopt           ││  ← radio-knoppen
│  │ ○ Pilot            ● Afgerond          ││
│  │ ○ Opschaling                                ││
│  │ ○ Gestopped                               ││
│  ├─────────────────────────────────────────┤│
│  │ Horizon              MDS team           ││
│  │ ○ H1     ○ H2  ○ H3  [Alle teams ▼]    ││  ← select dropdown
│  │ ○ Geen                                  ││
│  ├─────────────────────────────────────────┤│
│  │ Tags (meerdere mogelijk)               ││
│  │ ☑ AI            ☐ Duurzaamheid         ││  ← checkboxes
│  │ ☐ Participatie  ☐ Zelfbouw             ││
│  │ ☐ Digitaal      ☐ Dienstverlening      ││
│  │ ... (opvouwbaar bij veel tags)         ││
│  └─────────────────────────────────────────┘│
└─────────────────────────────────────────────┘
```
...
| Groep | Type | Selectie | Component |
|-------|------|----------|-----------|
| Fase | Enkel | Verkenning, Experiment, Pilot, Opschaling, Gestopt | Radio-knoppen |
| Status | Enkel | Actief, Gestopt, Afgerond | Radio-knoppen |
| Horizon | Enkel | H1, H2, H3, Geen | Radio-knoppen |
| MDS team | Enkel | Alle actieve teams | `<select>` dropdown |
| Tags | Meervoud | Alle actieve tags | Checkboxes (max 6 zichtbaar, "meer..." opvouwbaar) |

**Visuele feedback:**
- Badge bij de filterknop toont aantal actieve filters: `Filter resultaten (3)` — verdwijnt als er geen filters actief zijn.
- Actieve filters worden in het paneel gemarkeerd met de accentkleur.
- Knop "Alles wissen" rechts naast de filterknop verschijnt pas als er ≥1 filter actief is.
- Het paneel opent naar beneden (dropdown) en sluit bij klik buiten het paneel of Escape.

**Technische implementatie:**
- Client-side: HTMX `hx-get` naar `/api/initiatieven/filter` met query params.
- Server-side: bestaande filter-endpoint ondersteunt al `phase`, `status`, `horizon`, `tag_id`, `mds_id`, `search`.
- Extensie nodig: meerdere `tag_id` waarden ondersteunen (`tag_ids=uuid1,uuid2`) en `mds_id` filter toevoegen.
- De initiatievenlijst én het dashboard gebruiken hetzelfde filterpaneel-component (herbruikbaar partial template).
...
### F9 — Centrale vragen
- Apart scherm "Centrale vragen" in de hoofdnavigatie (onder "Overzicht").
- **CRUD:** centrale vraag aanmaken (vrij tekstveld + optionele toelichting), bewerken, en inactief zetten (soft delete).
- **Toelichting:** elk centrale vraag kan een markdown-toelichting hebben met uitleg over de vraag.
- **Bestanden:** bestanden kunnen worden geüpload bij een centrale vraag (max 25 MB per bestand), vergelijkbaar met dossier-bestanden.
- **Many-to-many relatie:** één initiatief kan meerdere centrale vragen hebben; één vraag kan bij meerdere initiatieven horen.
- **Bij initiatief aanmaken/bewerken:** dropdown/zoekinterface om bestaande actieve vragen te selecteren én optie om inline een nieuwe vraag toe te voegen.
- **Initiatief zonder centrale vraag is toegestaan;** het overzicht toont prominent hoeveel initiatieven nog geen centrale vraag hebben.
- **Inactieve vragen** (soft deleted) zijn niet zichtbaar in zoek/selectie, maar blijven historisch traceerbaar aan bestaande initiatieven.
- **Overzichtspagina:** lijst van actieve vragen met teller "gebruikt door X initiatieven"; klikken op een vraag toont alle gekoppelde initiatieven.
- Full-text search over vraagteksten en toelichtingen.

### F10 — AI-curatie-assistent (model-agnostisch)
- **Configuratie via omgevingsvariabelen:** `MODEL_URL`, `MODEL_NAME`, `MODEL_API_KEY` (optioneel), `AI_ENABLED`.
  Werkt met elk model dat een OpenAI-compatible chat completions endpoint ondersteunt (Ollama, vLLM, LM Studio, etc.).
- **Hypothese-suggesties:** op de tab "AI-assistent" van een initiatiefdetail kan de gebruiker hypothese-suggesties
  genereren. De AI analyseert titel, beschrijving, fase, horizon, MDS, bestaande hypothesen en centrale vragen,
  en stelt nieuwe hypothesen voor in de categorieën value/growth/compliance.
  Per suggestie kan direct "Toevoegen" worden geklikt om de hypothese aan het initiatief toe te voegen.
- **Narratief-generatie:** op een curatie-detailpagina kan een AI-narratief gegenereerd worden
  dat de curatie vertelt als samenhangend verhaal (3-5 alinea's), geschikt voor directievoordracht of show & tell.
  Het narratief is te kopiëren naar het klembord.
- **One-pager generatie:** op een initiatiefdetailpagina kan een korte one-pager samenvatting gegenereerd worden
  (max. ~400 woorden), gestructureerd als: Achtergrond → Doel → Stand van zaken → Verwachte impact.
- **Graceful degradation:** bij uitgeschakeld AI of onbereikbaar model toont de UI een duidelijke foutmelding
  zonder de rest van de applicatie te beïnvloeden.
- **Prompt templates** zijn centraal beheerd in `app/routes/ai.py` zodat ze eenvoudig aangepast kunnen worden.
...
| central_question | text nullable | legacy fallback, geleidelijk phase-out |
...
|---|---|----------|
| id | uuid | primary key |
| question | text | verplicht, de vraagstelling zelf |
| description | text nullable | toelichting, markdown |
| is_active | bool | default true; soft delete |
| created_at | timestamp |
| updated_at | timestamp |

### `central_question_files`
| veld | type | opmerking |
|---|---|----------|
| id | uuid | primary key |
| central_question_id | uuid fk | → central_questions |
| filename | text |
| mime_type | text |
| file_size | int | bytes |
| storage_path | text | relatief pad in lokale opslag |
...
### `initiative_questions` (join tabel)
| veld | type | opmerking |
|---|---|---|
| initiative_id | uuid fk | → initiatives |
| central_question_id | uuid fk | → central_questions |
| created_at | timestamp | wanneer de koppeling is gemaakt |

### `tags` (H2-1)
| veld | type | opmerking |
|---|---|---|
| id | uuid | primary key |
| name | text | verplicht, uniek; tag naam (bijv. "duurzaamheid") |
| created_at | timestamp | |

### `initiative_tags` (join tabel, H2-1)
| veld | type | opmerking |
|---|---|---|
| initiative_id | uuid fk | → initiatives |
| tag_id | uuid fk | → tags |

### `question_tags` (join tabel, H2-1)
| veld | type | opmerking |
|---|---|---|
| central_question_id | uuid fk | → central_questions |
| tag_id | uuid fk | → tags |
...
1. **AI-curatie-assistent.** ✅ F10 geïmplementeerd — model-agnostisch, configureerbaar via environment variabelen.
   Gegeven een beschrijving van wat je met een curatie wilt bereiken, suggests de AI welke initiatieven hierin passen op basis van titel, beschrijving, hypothesen en leeruitkomsten. Daarnaast: narratief-generatie (curatie → verhaalparagraaf; initiatief → one-pager voor directie) én **hypothese-suggesties** — bij een initiatief of curatie stelt de AI relevante hypothesen voor op basis van de context, domein en best practices.
   **Implementatie:** OpenAI-compatible API (werkt met Ollama, vLLM, LM Studio, etc.). Configureerbaar via `MODEL_URL`, `MODEL_NAME`, `AI_ENABLED`.
...
4. **Tags op initiatieven en centrale vragen.** ✅ Gereed.
   Thema-tags die zowel op initiatieven als op centrale vragen toegepast kunnen worden, voor betere grouping en filtering.
   **Datamodel:** tabel `tags` (id, name uniek, is_active), join-tabellen `initiative_tags` (initiative_id + tag_id) en `question_tags` (central_question_id + tag_id).
   **Gereed:** tag CRUD API, tags op initiatieven én centrale vragen, tag-selectie in alle relevante modals, tag-overzichtspagina met tellers, tag-detailpagina, filteren op tags in initiatievenlijst, FTS5 indexering, 10 seed-tags, 17 tests.
5. **MDS als entiteit.** In plaats van vrij tekstveld. Per MDS een dashboard.
...
4. **Aanvankelijke filterdiepte op de zoekfunctie.** Voorgesteld: fase, status, horizon, MDS, periode. Wil je ook kunnen filteren op *type hoofdhypothese* (alleen value, alleen compliance), *hypothese-status* (alleen weerlegd, om geleerde lessen te oogsten), of *centrale vraag*?
